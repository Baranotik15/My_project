...
class CartView(LoginRequiredMixin, ListView):
    ...
    def get_total_price(self):
        total_price = sum(item.product.price * item.quantity for item in self.get_queryset())
        return total_price
...
class UpdateCartItemQuantityView(LoginRequiredMixin, View):
    def post(self, request, cart_item_id, action):
        cart_item = get_object_or_404(CartItem, id=cart_item_id, cart__user=request.user, cart__is_active=True)
